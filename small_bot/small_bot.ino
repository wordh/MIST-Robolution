...
int rightMaxSpeed=110 ; // max speed of the robot
int leftMaxSpeed =110; // max speed of the robot
int rightBaseSpeed= 100; // this is the speed at which the motors should spin when the robot is perfectly on the line
int leftBaseSpeed =100;//this is the speed at which the motors should spin when the robot is perfectly on the line
...
// }else{
   readSensor();
//   if((!lff&&!rff)&&(lf||rf||rc||lc||rn||ln)){
//    straight_slight_turns();
//    Serial.println("Straight");
//    
//    }
     if(!lff&&!lf&&!ln&&!lc&&!rc&&!rn&&!rf&&!rff){
        u_turn();
...
void readSensor(){
   ...
   int lfar=analogRead(A0);
   int rfar=analogRead(A1);
 if(lfar>700)
  lfar=2500;
   if(rfar>=200)
   rfar=2500;
